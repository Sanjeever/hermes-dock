...
func (a *App) SaveEnvironmentForProfile(profileID string, vars []EnvVar) error {
	profileID, err := a.resolveProfileID(profileID)
	if err != nil {
		return err
	}
	if err := a.saveEnvironmentTo(a.profileEnvPath(profileID), vars); err != nil {
		return err
	}
...
func envOrder(key string) int {
	order := []string{
		"OPENCODE_GO_API_KEY",
		"DASHSCOPE_API_KEY", "ZHIPU_API_KEY", "ARK_API_KEY", "ARK_AGENT_PLAN_API_KEY", "DEEPSEEK_API_KEY", "AGNES_API_KEY",
