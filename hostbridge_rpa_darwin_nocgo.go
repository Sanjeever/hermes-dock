...
func hostRPAPlatformCapabilities() hostRPACapabilities {
	return hostRPACapabilities{Reason: "当前 macOS 构建未启用 CGO，无法使用桌面自动化", Backend: "none"}
}

func hostRPAListWindows() ([]hostRPAWindow, error) {
	return nil, errors.New("当前 macOS 构建未启用 CGO，无法使用桌面自动化")
}

func hostRPAActiveWindow() (hostRPAWindow, error) {
	return hostRPAWindow{}, errors.New("当前 macOS 构建未启用 CGO，无法使用桌面自动化")
}

func hostRPAActivateWindow(string) error {
	return errors.New("当前 macOS 构建未启用 CGO，无法使用桌面自动化")
}

func hostRPAMousePosition() (int, int, error) {
	return 0, 0, errors.New("当前 macOS 构建未启用 CGO，无法使用桌面自动化")
}
