...
~/.hermes-dock/launcher/runtime-deps/<version>/ 当前平台内置 Python wheelhouse
...
- `.hdbackup` 只提供快速迁移备份，不提供完整备份模式；保留配置、凭据、业务数据和用户项目，跳过可重建依赖、缓存、临时文件和检查点。备份包含 `.env` 密钥、平台账号凭据、Web 管理配置和 Dufs 账号密码哈希；UI 必须明确提示备份文件包含敏感信息。
...
- 快速迁移备份不包含 `shared`、`launcher/backups`、`launcher/logs`、`launcher/web-sessions.json`、`launcher/apply-status.json`、`launcher/host-bridge.token`、`data/.dock` 派生运行态、缓存、虚拟环境、`node_modules`、临时文件、模型列表缓存或检查点；不得通用排除 `.git`、`build` 或 `dist`，避免遗漏用户源码和交付物。导入后为当前设备重新生成 Host Bridge Token；必须继续支持导入旧版包含可重建目录的 `.hdbackup`。
...
- Web 管理与桌面端保持同等管理能力和状态语义。认证后的 Web RPC 可以返回和编辑当前 profile 的完整环境、模型及供应商配置，其中包含 API Key 等敏感值；访问密码和登录会话是远程管理边界。敏感值不得提供给未认证请求、写入 Web 日志或保存到非预期的浏览器持久化位置。
...
- 标准 Compose 不发布 Hermes 的 8642/9119 端口，Hermes 原生 Dashboard 固定关闭。
- Hermes 与 Dufs 使用不同的用户自定义 bridge 网络；二者只共享宿主机文件目录，不共享容器网络。
- 中国大陆友好的 pip、uv、npm 镜像源。
- 数据目录权限依赖 Hermes 镜像启动脚本定向处理，不要增加每次启动都对整个 `/opt/data` 执行 `chown -R` 的 init service。
- 飞书和钉钉的 CPython 3.13 Linux wheels 随 Hermes Dock 发布产物提供；Windows/Linux amd64 只内置 `linux/amd64`，macOS arm64 只内置 `linux/arm64`。wheel 文件使用 Git LFS 管理，发布构建必须拉取当前目标架构的 LFS 内容。
- 内置 wheelhouse 释放到 `launcher/runtime-deps/<version>/` 并只读挂载到 `/opt/hermes-dock/runtime-deps`；`launcher/helpers/verify-runtime-deps` 挂载到 `/etc/cont-init.d/016-verify-runtime-deps`，先校验 Python ABI、容器架构和 `SHA256SUMS`，失败时禁止继续安装。中断的释放 staging 在下次释放前清理，旧版本 bundle 只能在新 Hermes 容器确认成功后删除。
- 飞书和钉钉依赖分别由 `/etc/cont-init.d/018-install-feishu-deps`、`/etc/cont-init.d/020-install-dingtalk-deps` 使用 `uv --offline --no-index` 安装到 `/opt/hermes/.venv`，不得访问包索引或回退到互联网。
- 固定 Hermes 镜像的钉钉本地媒体发送由 `launcher/helpers/patch-dingtalk-media` 挂载到 `/etc/cont-init.d/021-patch-dingtalk-media` 实现；补丁必须校验目标源码指纹、保持幂等并在不匹配时失败。只支持当前收到消息的单聊或群聊发送不超过 20 MiB 的 JPG、JPEG、PNG、GIF 图片、音频、视频和带扩展名的普通文件；缺少原生音视频所需的时长或封面元数据时作为普通文件发送，不承诺缺少入站消息上下文的主动媒体发送。
- 图片文字 OCR 运行依赖不随启动器发布，也不在容器启动时安装；`image-text-ocr` 技能内置 PP-OCRv6_small 检测和识别模型，首次识别时由技能包装脚本联网下载经过版本和 wheel 哈希锁定的依赖并安装到 `data/.dock/image-text-ocr-venv`，后续复用。运行时不得下载模型或把图片发送到外部服务。
- 简单静态图片验证码由 `captcha-ocr` 技能使用 ddddocr beta 模型识别，不再使用通用文档 OCR；首次识别时由技能包装脚本联网下载包含模型且经过版本和 wheel 哈希锁定的依赖并安装到 `data/.dock/captcha-ocr-venv`，后续复用。只允许处理字符和基础算术验证码，不得用其滑块、点选或拼图能力绕过反自动化验证。
- Hermes 镜像内置的 Playwright Chromium 通过 Compose 的 `AGENT_BROWSER_EXECUTABLE_PATH` 显式提供给 `agent-browser`，确保默认 `browser` toolset 无需首次下载即可使用；升级固定 Hermes 镜像时必须同步核对该路径。
...
“应用配置”必须比较标准 Compose 与 override 的当前指纹和 `LastAppliedComposeHash`。仅 profile `.env`、`config.yaml`、`SOUL.md`、skills、平台绑定或启停状态变化时，复用现有容器并执行 `docker compose restart hermes`；镜像、资源、代理、共享目录、标准 Compose 或 override 变化时执行重建。容器不存在或状态未知时也执行重建。
...
- 认证后的 Web 版 `GetAppStateForProfile` 与桌面端返回相同的 `AppState`，包括当前 profile 的完整 `Environment`、`Model` 和 `Providers`；不得为了 Web 运行时静默删减或掩码这些字段，导致桌面端与 Web 端行为或语义不一致。
...
- runner 必须安全解析每个 profile 的 `.env`，禁止用 shell `source` 执行用户可编辑文件；合并后必须重新写入 `HERMES_HOME`、`HERMES_DOCK_PROFILE`、`HERMES_DOCK_PROFILE_HOME`、`HERMES_WRITE_SAFE_ROOT` 和 `HERMES_DASHBOARD` 等保留运行时变量，不允许 `.env` 覆盖。
...
- 启动器内置 `SOUL.md` 和 skills 支持批量同步：同步内置人格时先备份再直接重置 `SOUL.md`；内置 skills 新增缺失文件、保持内容一致的文件不变，并用当前模板覆盖内容不同的同名文件，同时保留自定义 skills 和模板已移除的旧 skills。
- 软件升级后会对所有 profile 执行一次内置内容同步：`SOUL.md` 只更新有可信内置基线且未被用户修改的文件；内置 skills 新增缺失文件、保持内容一致的文件不变，并用当前模板覆盖内容不同的同名文件，同时保留自定义技能。如有写入，且 Hermes 在升级前和应用前都确认为 `running`，自动启动“应用配置”；Hermes 已停止或状态未知时不得自动启动。
...
- 钉钉本地图片、音频、视频和普通文件通过 `/etc/cont-init.d/021-patch-dingtalk-media` 补丁使用媒体上传和 Robot OpenAPI 发送；只允许当前收到消息的会话，不记录 access token 或 media ID，失败时必须向用户显示明确提示。
...
- 检测到新版本后，“立即更新”会下载平台安装包和 `SHA256SUMS.txt`，校验后由独立 `hermes-dock-updater` 在主程序退出后安装并重启；安装阶段不得停止 Hermes Docker 容器。新版本通过启动健康确认后，再后台同步所有 profile 的内置人格和技能；内置技能会用当前模板覆盖内容不同的同名文件。仅当同步产生变化，且 Hermes 在升级前和应用前都确认为 `running` 时自动应用配置，否则不重启或自动启动服务。
...
- `runtime-deps/**/*.whl` 使用 Git LFS；发布 workflow 只拉取目标产物对应的 Linux 架构 wheelhouse，并在 Go 构建时嵌入主程序。不得把 LFS pointer 当作 wheel 打包。
...
- 图片文字 OCR 按需安装和执行入口位于内置 `image-text-ocr` 技能的 `scripts/run_ocr.py`；依赖版本或 wheel 哈希变化时必须同步更新 `scripts/requirements.lock`，脚本或内置模型修改后必须同步更新 skill 同步相关测试。
- 图片验证码识别按需安装和执行入口位于内置 `captcha-ocr` 技能的 `scripts/run_ocr.py`；依赖版本或 wheel 哈希变化时必须同步更新 `scripts/requirements.lock`，脚本修改后必须同步更新 skill 同步相关测试。
- 运行依赖校验 helper 来源是 `launcher/helpers/verify-runtime-deps`；更新 wheelhouse 时必须同步更新锁文件和 `SHA256SUMS`，并递增 `runtimeDependencyBundleVersion`。
