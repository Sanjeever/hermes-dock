...
- 整实例备份不包含 `shared`、`launcher/backups`、`launcher/logs`、`launcher/web-sessions.json`、`launcher/host-bridge.token` 或 `data/.dock` 派生运行态；导入后为当前设备重新生成 Host Bridge Token。
...
- 官网自动部署只上传 `dist/`、`server/`、`package.json` 和 `pnpm-lock.yaml`，不得上传或覆盖服务器上的 `/opt/qizhih-website-server/docker-compose.yaml`。
- 官网采用覆盖式部署：替换 `/home/nginx/html/qizhih-website`，停止 API 容器，删除 `/opt/qizhih-website-server/node-modules`、`server`、`package.json` 和 `pnpm-lock.yaml` 后上传新文件并重新启动。该流程有短暂停机且不自动回滚，失败时应通过 Actions 日志和手工部署流程恢复。
