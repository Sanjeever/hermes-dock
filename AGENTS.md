...
website/                   独立官网、预约演示 Worker 和 Cloudflare 部署配置
app.go                     Wails 状态聚合
...
- `website/` 独立部署到 Cloudflare Workers，静态页面和 `POST /api/demo-requests` 使用同一 Worker；预约通知通过外部 SMTP 发送，不依赖桌面主进程。
- 官网 SMTP 使用 `SMTP_HOST`、`SMTP_PORT`、`SMTP_SECURE`、`SMTP_USER`、`SMTP_PASS`、`SMTP_FROM` 和 `MAIL_TO` 配置；真实值只放 `.dev.vars` 或 Cloudflare Variables/Secrets，不提交到 Git。
- 官网 `SMTP_SECURE=true` 表示连接即 TLS，`false` 表示必须升级 STARTTLS，不允许降级到明文；Cloudflare Workers 不支持 SMTP 25 端口。
- 官网发件人和收件人只从 Worker 环境读取，不接受前端请求指定；SMTP 完成并明确接受邮件后才向页面返回成功。
