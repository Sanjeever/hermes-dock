...
- 固定 Hermes 镜像的钉钉本地图片发送由 `launcher/helpers/patch-dingtalk-images` 挂载到 `/etc/cont-init.d/021-patch-dingtalk-images` 实现；补丁必须校验目标源码指纹、保持幂等并在不匹配时失败。只支持当前收到消息的单聊或群聊发送不超过 20 MiB 的 JPG、JPEG、PNG、GIF 图片，不承诺普通文件、语音、视频或缺少入站消息上下文的主动图片发送。
...
- 钉钉本地图片通过 `/etc/cont-init.d/021-patch-dingtalk-images` 补丁使用媒体上传和 Robot OpenAPI 发送；只允许当前收到消息的会话，不记录 access token 或 media ID，失败时必须向用户显示明确提示。
