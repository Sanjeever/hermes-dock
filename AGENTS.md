...
- 图片文字 OCR 运行依赖通过 `launcher/helpers/install-paddleocr-deps` 挂载到 `/etc/cont-init.d/021-install-paddleocr-deps`，安装到 `data/.dock/image-text-ocr-venv`；`image-text-ocr` 技能内置 PP-OCRv6_small 检测和识别模型，运行时不得下载模型。
