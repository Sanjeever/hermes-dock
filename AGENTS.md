...
- 官网自动部署上传 `dist/` 和由 esbuild 生成的 Node API 单文件 bundle，不得上传或覆盖服务器上的 `/opt/qizhih-website-server/docker-compose.yaml`、`package.json`、`pnpm-lock.yaml`、`node-modules/` 或 `server/` 中除 `index.js` 外的文件。
- 官网采用覆盖式前端部署和单文件 API 部署：替换 `/home/nginx/html/qizhih-website`，停止 API 容器，原子替换 `/opt/qizhih-website-server/server/index.js` 后重新启动。服务器目录结构保持不变。该流程有短暂停机且不自动回滚，失败时应通过 Actions 日志和手工部署流程恢复。
