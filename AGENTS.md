...
- 飞书、钉钉和 PaddleOCR 的 CPython 3.13 Linux wheels 随 Hermes Dock 发布产物提供；Windows/Linux amd64 只内置 `linux/amd64`，macOS arm64 只内置 `linux/arm64`。wheel 文件使用 Git LFS 管理，发布构建必须拉取当前目标架构的 LFS 内容。
...
- 图片文字 OCR 运行依赖通过 `/etc/cont-init.d/021-install-paddleocr-deps` 从内置 wheels 安装到 `data/.dock/image-text-ocr-venv`；`image-text-ocr` 技能内置 PP-OCRv6_small 检测和识别模型，运行时不得下载依赖或模型。
...
- 图片文字 OCR 依赖 helper 来源是 `launcher/helpers/install-paddleocr-deps`，脚本或内置模型修改后必须同步更新 helper 释放、Compose 迁移和 skill 同步相关测试。
