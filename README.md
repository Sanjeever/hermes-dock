...
固定 Hermes 镜像的钉钉适配器不支持本地图片上传。`/etc/cont-init.d/021-patch-dingtalk-images` 会在容器启动时校验目标源码指纹并应用幂等补丁，通过钉钉媒体上传和 Robot OpenAPI 为当前收到消息的单聊或群聊发送不超过 20 MiB 的 JPG、JPEG、PNG、GIF 图片。补丁不记录 access token 或 media ID；目标源码不匹配时启动失败，不静默跳过。普通文件、语音、视频和缺少当前入站消息上下文的主动图片发送不在该补丁范围内。
