...
- 单聊响应每条消息并保持独立会话；群聊仅在机器人被 `@` 时响应。
- `group_sessions_per_user=false`，同一群内的所有用户共享一份对话历史。该设置作用于当前 profile 的所有群聊平台。
- 钉钉默认显示模型推理并启用流式响应，不显示工具执行进度和中间注释消息。

钉钉高级配置可选填 AI 卡片模板 ID；填写后使用流式 AI 卡片回复，留空时使用 Markdown。模板 ID 由每个钉钉应用在开发者控制台中创建，Dock 不提供通用默认值，也可在未绑定钉钉时独立保存。取消绑定会备份并清除应用专属的模板 ID，避免后续绑定的新应用继承旧配置。新 profile 自动使用上述推荐行为；已有 profile 不会被静默改写，可在钉钉高级配置中显式应用推荐设置，写入前会备份 `config.yaml`。推荐设置还会开启 profile 级流式总开关，因此也可能启用同一 profile 中其他已配置为流式的平台。
...
固定 Hermes 镜像的钉钉适配器不支持本地媒体上传。`/etc/cont-init.d/021-patch-dingtalk-media` 会在容器启动时校验目标源码指纹并应用幂等补丁，通过钉钉媒体上传和 Robot OpenAPI 为当前收到消息的单聊或群聊发送不超过 20 MiB 的 JPG、JPEG、PNG、GIF 图片、音频、视频和带扩展名的普通文件。缺少原生音视频所需的时长或封面元数据时会作为普通文件发送。补丁不记录 access token 或 media ID；目标源码不匹配时启动失败，不静默跳过。缺少当前入站消息上下文的主动媒体发送不在该补丁范围内。
