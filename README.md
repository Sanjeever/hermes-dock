...
“应用配置”会比较标准 Compose 和 override 的当前指纹与上次成功应用的指纹。只修改 profile 的 `.env`、`config.yaml`、`SOUL.md`、skills、平台绑定或启停状态时，复用现有容器并重启 `hermes` 服务；镜像、端口、资源、代理或 Compose override 变化时才重新创建容器。两种路径都会等待当前 runtime generation 上报就绪后再标记应用成功。
