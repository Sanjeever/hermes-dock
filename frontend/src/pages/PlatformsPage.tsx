...
export function PlatformsPage(props: {
    ...
    const wecomDMPolicy = closedPolicyValue(envValue(props.env, 'WECOM_DM_POLICY'));
    const wecomGroupPolicy = closedPolicyValue(envValue(props.env, 'WECOM_GROUP_POLICY'));
    const feishuDomain = enumValue(envValue(props.env, 'FEISHU_DOMAIN'), ['feishu', 'lark'], 'feishu');
    const feishuGroupPolicy = disabledPolicyValue(envValue(props.env, 'FEISHU_GROUP_POLICY'));
    return (
