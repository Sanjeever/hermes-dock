...
同步内置技能会新增缺失文件、保持内容一致的文件不变，并在备份后用当前模板覆盖内容不同的同名内置文件；自定义技能和模板外文件会保留。批量删除会逐个备份所选技能；恢复默认技能则会先备份整个技能目录，再删除全部现有技能并重新释放内置默认技能。

### 图片文字识别

企智盒内置本地图片文字识别技能，可提取截图、照片和图片附件中的可见文字。PP-OCRv6_small 模型随企智盒提供；首次识别需要联网下载经过版本和哈希锁定的 PaddleOCR 运行依赖，安装成功后可离线复用。识别不会下载模型，也不会把图片发送到外部服务。

图片文字识别只负责读取文字，不能替代物体、人物、颜色、布局含义或图表趋势等视觉理解。缺少该技能时先同步最新内置技能；首次依赖安装失败时，根据错误检查容器网络或代理设置后重试识别。
...
钉钉支持助手在当前对话中发送不超过 20 MiB 的 JPG、JPEG、PNG 或 GIF 图片。普通文件、语音、视频以及软件重启后缺少当前对话上下文的主动图片发送暂不支持，可改用共享文件目录交付。
...
“检查更新”用于获取公开发布的新版本。手动更新会校验安装包后退出应用并完成安装，安装阶段不停止正在运行的 Docker 服务。新版本启动后会同步所有助手的内置人格和技能，其中内置技能会用当前模板覆盖内容不同的同名文件，并保留自定义技能。如果同步产生变化，且 Hermes 在升级前和应用前都确认正在运行，启动器会自动应用配置，期间可能短暂重启或重建 Hermes；已停止或状态未知的服务不会被自动启动。失败时可在“设置 → 软件更新”重新处理。
