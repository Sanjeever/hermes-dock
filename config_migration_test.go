...
func TestAddProfileStreamingDefaultsPreservesExistingValues(t *testing.T) {
...
`)
	...
	if !changed {
		t.Fatal("missing streaming defaults should be added")
	}
	...
	if asBool(streaming["enabled"]) {
		t.Fatal("existing streaming.enabled should be preserved")
	}
	...
	if asBool(asMap(platforms["feishu"])["streaming"]) {
		t.Fatal("existing feishu streaming setting should be preserved")
	}
...
func TestMigrateProfileStreamingDefaultsUpdatesAllProfiles(t *testing.T) {
	...
	for _, backup := range state.Backups[backupsBefore:] {
		if backup.Reason != "before-profile-streaming-migration" {
