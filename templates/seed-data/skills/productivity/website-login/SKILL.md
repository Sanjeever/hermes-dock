---
name: website-login
description: 使用 Hermes 浏览器工具安全地登录网站。适用于用户提供登录网址及账号、密码等必要信息，并要求打开后台、登录系统、处理短信或邮件验证码、扫码确认，或识别字符及算术题等简单图片验证码的场景。图片验证码必须等待渲染完成后对验证码 DOM 元素截图，再交给视觉模型或本地 OCR；不使用整页截图、canvas 导出或 base64 提取代替元素截图。支持在人工验证码处暂停并等待用户继续；不用于绕过滑块、reCAPTCHA、设备验证、安全控件或其他反自动化机制。
---
...
1. 对验证码 DOM 的元素截图直接调用 `vision_analyze`，要求只返回图片中实际显示的验证码字符或完整算式。不要先调用本地 OCR，也不要使用整页 `browser_vision` 识别验证码。
